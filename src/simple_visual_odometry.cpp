  ...
  nh(n),  pnh(p), it(p)
{
  ...
  int avg_x, avg_y, avg_th;
  pnh.getParam("average/x", avg_x);
  pnh.getParam("average/y", avg_y);
  pnh.getParam("average/theta", avg_th);
  avg_dx.setFilterLength(avg_x);
  avg_dy.setFilterLength(avg_y);
  avg_theta.setFilterLength(avg_th);
...
void SimpleVisualOdometry::imageCb(const sensor_msgs::ImageConstPtr& msg)
{
  ...
  if(SUCCESS != featureTracking(roi, newImage->image, oldImage->image))
  {
    ROS_WARN("Problem in Feature Tracking. Skip this image");
    return;
  }
  ...
  if(newImagePoints.size() > 1){
      ...
      if(cv::solve(leftSide,rightSide,res,cv::DECOMP_SVD)){
          float dx =       avg_dx.addAndGetCrrtAvg(res.at<double>(2));
          float dy =       avg_dy.addAndGetCrrtAvg(res.at<double>(3));
          float theta = avg_theta.addAndGetCrrtAvg(std::atan2(res.at<double>(1), res.at<double>(0)));
...
bool SimpleVisualOdometry::featureTracking(cv::Rect roi, const cv::Mat& new_im, const cv::Mat& old_im){
    newImagePoints.clear();
    for(cv::Point2f &v:oldImagePoints){
        v.x -= roi.x;
        v.y -= roi.y;
    ...
    return checkNewFeaturePoints(roi);
}

bool SimpleVisualOdometry::checkNewFeaturePoints(const cv::Rect roi){
    if(newImagePoints.size() != oldImagePoints.size()){
        ROS_ERROR( "newImagePoints size does not match oldImagePoints size" );
        return FAIL;
    }
